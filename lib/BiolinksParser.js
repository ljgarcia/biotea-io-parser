        ...
        if (article.id !== queryArticle.id) {
            ...
            if (groupArray) {
                ...
                _.each(groupArray, function(group) {
                    var grpScore = _.find(queryArticle.distribution, function(dist) {
                        return dist.group === group;
                    });
                    if (grpScore && (grpScore.score !== 0)) {
                        weight *= grpScore.score;
                        var grpCompScore = _.find(article.distribution, function(dist) {
                            return dist.group === group;
                        });
                        weight *= grpCompScore && (grpCompScore.score !== 0) ? grpCompScore.score : 1;
                    }
                });
                similarity.score = ratio === 0 ? 0 : weight  * (numerator / ratio);
            } else {
