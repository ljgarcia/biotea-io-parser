...
var calculateTotalEntropy = function(parser, distribution, useGroups, article, totalTF) {
...
BiolinksParser.prototype.calculateDistribution = function(annotatedArticles, includeGroups) {
    var parser = this;
    var allDistribution = [];
    ...
    _.each(annotatedArticles, function(article) {
        ...
        var calculation = calculateTotalEntropy(parser, distribution, useGroups, article, totalTF);
...
BiolinksParser.prototype.calculateSimilarity = function(queryArticle, allRelatedArticles, includeGroups) {
    ...
    var fullTotalFrequencyQuery = calculateTotalTF(queryArticle, undefined);
    var groupsTotalFrequencyQuery = groupArray ? calculateTotalTF(queryArticle, groupArray)
    ...
    _.each(allRelatedArticles, function(article) {
        if (article.id !== queryArticle.id) {
            ...
            var fullTotalFrequencyCompared = calculateTotalTF(article, undefined);
            var groupsTotalFrequencyCompared = groupArray ? calculateTotalTF(article, groupArray)
            ...
            if (groupArray) {
                ...
                similarity.score = ratio === 0 ? 0 : (weightCompared / weight)  * (numerator / ratio);
            } else {
